...
type DemoComsumerStruct struct {
	Consumer
}

var DemoComsumer *DemoComsumerStruct
...
func NewComsumer(appID string) *DemoComsumerStruct {
	...
	dis := naming.New(conf)
	demoComsumer := &DemoComsumerStruct{Consumer{
		Conf:  conf,
		AppID: appID,
		Dis:   dis.Build(appID),
	}}
...
func (c *DemoComsumerStruct) getInstances(ch <-chan struct{}) {
